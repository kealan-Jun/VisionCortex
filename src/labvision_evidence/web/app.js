...
function renderTasks() {
  ...
  main.innerHTML = `<div class="page"><header class="page-hero compact"><div><p class="eyebrow">LIVE OPERATIONS</p><h1>任务进度</h1><p>上传、原视频留存和全部分析阶段的实时状态。服务重启前的内存任务会显示在这里；正式结果始终保留在 NAS 实验记录。</p></div><div class="hero-actions"><a class="primary-button" href="#/new">${icon("plus")}新建实验</a></div></header><section class="status-grid">${statusCard("activity","当前任务",number(runs.length),"本次服务会话")}${statusCard("gauge","正在处理",number(runs.filter((run)=>!["completed","failed"].includes(run.state)).length),"后台自动执行")}${statusCard("check","已完成",number(runs.filter((run)=>run.state==="completed").length),"已进入正式档案")}${statusCard("file","失败",number(runs.filter((run)=>run.state==="failed").length),"保留诊断信息")}</section><section class="panel"><header class="panel-heading"><div><h2>运行队列</h2><p>每个任务完成一项就向 NAS 发布一项。</p></div></header>${runs.length ? `<div class="queue-list">${runs.map((run)=>`<article class="queue-card"><span class="queue-status ${esc(run.state)}"><i></i>${esc(STAGE_LABELS[run.state]||run.state)}</span><span class="archive-identity"><strong>${esc(run.experiment_id||run.run_id)}</strong><small>${esc(run.message||run.run_id)}</small></span><span class="queue-progress"><small>${Math.round(Number(run.progress||0)*100)}%</small><span><i style="width:${Math.round(Number(run.progress||0)*100)}%"></i></span></span>${run.nas_output?`<span class="row-link">${esc(run.nas_output)}</span>`:""}</article>`).join("")}</div>`:`<div class="empty-state"><strong>当前没有运行任务</strong><p>从“新建实验”上传视频后，任务会立即出现在这里。</p></div>`}</section></div>`;
}
...
function metricsView(data) {
  ...
  return `<section class="panel"><header class="panel-heading"><div><h2>阶段耗时</h2><p>完整预处理基准为 ${duration(metrics.display_preprocessing_seconds)}，明确不包含模型理解；当前运行流水线耗时 ${duration(metrics.total_duration_seconds)}${endToEnd?.total_duration_seconds != null ? `，端到端 ${duration(endToEnd.total_duration_seconds)}` : ""}。</p></div></header><table class="metric-table"><thead><tr><th>阶段</th><th>说明</th><th>耗时</th></tr></thead><tbody>${extraRows}${stages.map((stage)=>`<tr><td>${esc(stage.stage)}</td><td>${esc(STAGE_LABELS[stage.stage] || stage.stage)}</td><td>${duration(stage.duration_seconds)}</td></tr>`).join("")}</tbody></table></section><section class="panel"><header class="panel-heading"><div><h2>Token 用量</h2><p>CV、FFmpeg 与 TensorRT 不消耗模型 Token；仅多模态步骤理解计费。</p></div></header><table class="metric-table"><thead><tr><th>阶段</th><th>调用次数</th><th>输入 Token</th><th>输出 Token</th><th>总 Token</th></tr></thead><tbody><tr><td>实验片段步骤理解</td><td>${number(tokens.experiment_groups?.call_count)}</td><td>${number(tokens.experiment_groups?.input_tokens)}</td><td>${number(tokens.experiment_groups?.output_tokens)}</td><td>${number(tokens.experiment_groups?.total_tokens)}</td></tr><tr><td>关键素材理解</td><td>${number(tokens.key_materials?.call_count)}</td><td>${number(tokens.key_materials?.input_tokens)}</td><td>${number(tokens.key_materials?.output_tokens)}</td><td>${number(tokens.key_materials?.total_tokens)}</td></tr><tr><td><strong>全任务</strong></td><td>—</td><td><strong>${number(tokens.run_total?.input_tokens)}</strong></td><td><strong>${number(tokens.run_total?.output_tokens)}</strong></td><td><strong>${number(tokens.run_total?.total_tokens)}</strong></td></tr></tbody></table></section><section class="panel"><header class="panel-heading"><div><h2>模型理解与验收文件</h2><p>这些链接直接指向当前 NAS 档案中的正式文件。</p></div></header><div class="result-links" style="padding:20px">${Object.entries({experiment_understanding:"实验级步骤理解 JSON",key_material_understanding:"关键素材模型理解 JSON",metrics:"耗时与 Token JSON",acceptance:"验收报告 JSON"}).map(([key,label])=>`<a target="_blank" href="${esc(data.links[key])}">${icon("file")}${label}</a>`).join("")}</div></section>`;
}
