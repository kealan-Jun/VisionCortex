...
def load_dataset_scoped_json(
    configured: str | Path | dict[str, str | Path] | None,
    ...
    *,
    ...
    configured_map: dict[str, str | Path] | None = None
    if isinstance(configured, dict):
        configured_map = configured
        selected = configured.get(str(current_experiment_id))
        if selected is None:
        ...
        if selected is None:
            return None, {
                "configured": True,
                "applied": False,
                "reason": "no_artifact_configured_for_experiment_id",
                "current_experiment_id": current_experiment_id,
                "configured_experiment_ids": sorted(
                    key for key in configured if key != "default"
                ),
            }
        configured = selected
    ...
    common = {
        ...
        "configured_via_dataset_map": configured_map is not None,
    }
    if not allowed_ids:
        return None, {
            **common,
