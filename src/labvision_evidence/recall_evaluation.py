...
# These two classes are directly observable from tracked object/hand geometry.
# A whole multi-view claim can remain uncertain when synchronized cameras show
# different workers, while one view still proves the action class itself.
# Indirect physical-state classes deliberately remain strict.
DIRECT_ACTION_CLASSES = {
    "hand_object_contact",
    "object_movement",
}
...
def evaluate_key_event_recall(
    ...
    *,
    ...
    cv_only_predictions: list[dict[str, Any]] = []
    excluded_prediction_ids: list[str] = []
    excluded_semantically_unconfirmed_prediction_ids: list[str] = []
    ...
    for event in predictions:
        ...
        cv_only_predictions.append(cv_prediction)
        semantic_review = event.semantic_review or {}
        semantic_verdict = str(semantic_review.get("verdict") or "")
        if semantic_review and semantic_verdict != "confirmed":
            action_type = _canonical_action_type(event.action_type)
            model_action_type = _canonical_action_type(
                semantic_review.get("model_action_type")
            )
            model_verdict = str(
                semantic_review.get("model_evidence_verdict") or ""
            ).strip().lower()
            model_status = str(
                semantic_review.get("model_status") or ""
            ).strip().lower()
            model_confidence = float(
                semantic_review.get("model_confidence") or 0.0
            )
            per_view_observations = list(
                (event.model_understanding or {}).get(
                    "per_view_observations"
                )
                or []
            )
            direct_class_supported = all(
                (
                    action_type in DIRECT_ACTION_CLASSES,
                    model_action_type == action_type,
                    model_status == "completed",
                    model_verdict in {"uncertain", "relabel_suggested"},
                    model_confidence >= 0.55,
                    any(
                        str(item.get("observation") or "").strip()
                        for item in per_view_observations
                        if isinstance(item, dict)
                    ),
                )
            )
            if direct_class_supported:
                direct_action_class_supported_prediction_ids.append(
                    event.event_id
                )
            else:
                excluded_semantically_unconfirmed_prediction_ids.append(
                    event.event_id
                )
                continue
        if not semantic_review:
            prediction_ids_without_semantic_review.append(event.event_id)
        normalized_predictions.append(cv_prediction)
    ...
    def threshold_reports_for(
        evaluated_predictions: Sequence[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        action_types = sorted(
            {
                item["action_type"]
                for item in [*normalized_labels, *evaluated_predictions]
                if item["action_type"]
            }
        )
            ...
            reports.append(
                {
                    ...
                    **_metric_summary(
                        len(matches),
                        len(evaluated_predictions) - len(matches),
    ...
    threshold_reports = threshold_reports_for(normalized_predictions)
    cv_only_threshold_reports = threshold_reports_for(cv_only_predictions)
    ...
    return {
        ...
        "matching_policy": {
            ...
            "semantically_reviewed_unconfirmed_predictions": "excluded",
            "direct_action_class_with_multiview_uncertainty": (
                "hand_object_contact/object_movement retained only when the "
                "completed model review confirms the same class at confidence "
                ">=0.55 with a non-empty per-view observation; the broader "
                "semantic claim remains uncertain"
            ),
            "predictions_without_semantic_review": (
                "retained_as_cv_predictions_and_reported_separately"
            ),
            "alignment": (
        ...
        "prediction_count_in_scope": len(normalized_predictions),
        "cv_prediction_count_in_scope": len(cv_only_predictions),
        ...
        "excluded_semantically_unconfirmed_prediction_ids": (
            excluded_semantically_unconfirmed_prediction_ids
        ),
        "direct_action_class_supported_prediction_ids": (
            direct_action_class_supported_prediction_ids
        ),
        "prediction_ids_without_semantic_review": (
            prediction_ids_without_semantic_review
        ),
        "threshold_results": threshold_reports,
        "cv_only_threshold_results": cv_only_threshold_reports,
    }
