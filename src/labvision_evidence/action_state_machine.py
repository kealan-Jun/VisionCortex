...
from .schemas import ActionCandidate, ActionType, EvidenceEvent, ViewRole
...
def _phase_contract(
    ...
    if event.action_type == ActionType.HAND_OBJECT_CONTACT:
        ...
        after = {
            ...
            "source": "contents_interacted" if present else "unknown",
            "target": (
                "contents_interacted" if flags["transfer_sequence"] else "not_observed"
            ),
        }
        ...
        if flags["state_change"] or {"reagent_bottle_open", "bottle_cap"} <= objects:
...
def attach_continuous_action_states(
    ...
    for component in events:
        ...
        for primary in events:
            if not primary.accepted or primary.action_type not in primary_types:
                continue
