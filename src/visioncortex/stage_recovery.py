...
OUTPUTS = {
    ...
    "candidate_fine": "detection_paths exhaustive_full_timeline fine_runtime_index progressive_report scanned_fine_ids candidates fine_views fine_view_report fine_windows",
...
class StageRunner:
    ...
    def _inventory(self):
        ...
        inventory = self.layout.root / "StageInventory.json"
    ...
    def _start_speech(self):
        executor = getattr(self.pipeline, "_stage_executor", None)
        if executor is None or self.outcomes.get("alignment") not in {"completed", "skipped"}:
            return
        key = self.request_key("speech")
        self.request_keys["speech"] = key
        if self.pipeline._stage_outcomes.get("alignment", {}).get("recomputed"):
            self.saved.pop("speech", None)
        if self.reuse("speech", key):
            return
        self.pipeline._speech_future = executor.submit(
            "stt", self.pipeline._run_speech_stage, self.layout, self.manifest,
            self.context.infos, self.context.transforms,
        )

    def run(self):
        stages = list(DEPENDENCIES)
        stages.remove("speech")
        stages.insert(stages.index("experiment_understanding"), "speech")
        for number, stage in enumerate(stages):
            dependencies = DEPENDENCIES[stage]
            if stage == "speech" and stage in self.outcomes:
                continue
