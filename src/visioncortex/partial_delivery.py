...
def registered_partial_root(config: dict[str, Any], record: dict[str, Any]) -> Path | None:
    ...
    try:
        ...
        if partial.get("formal_archive_promotion_allowed") is not False:
            return None
        if record.get("state") == "partial" and not partial_result_available(root):
            return None
...
def register_partial_result(root: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if status.get("stage") not in {"failed", "interrupted", "partial"}:
        raise ValueError("Only stopped partial results may be registered")
    if status.get("stage") == "partial" and not partial_result_available(root):
        raise ValueError("Quality-attention result has no validated partial delivery")
