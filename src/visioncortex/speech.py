...
def runtime_request(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if options.get("provider") == "aliyun_qwen":
        from . import speech_qwen
        return speech_qwen.runtime_request(config)
...
def invoke(directory: Path, request: dict[str, Any], *, reuse: bool = True, config=None) -> dict[str, Any]:
    if request.get("provider") == "aliyun_qwen":
        from . import speech_qwen
        from .provider_control import provider_request
        with provider_request(config or {}, 'stt', request.get('credential_connection', {})):
            return speech_qwen.invoke(directory, request, reuse=reuse)
    ...
    from .owned_subprocess import run as run_owned
    result = run_owned(
        [
    ...
    try:
        ...
        for item, public in zip(sources, index["sources"], strict=True):
            ...
            for chunk_index in range(count):
                ...
                receipt = invoke(work, request, reuse=config.get("project", {}).get("cache_mode") != "cold", config=config)
                ...
                for name in (
                    ...
                    "receipt.json",
                    "request.json",
                    *(["response.json"] if "response.json" in receipt.get("artifacts", {}) else []),
                ):
