...
def validate_final_step_action_consistency(
    ...
    for group in groups:
        ...
        for step in steps:
            ...
            step_actions = {event_by_id[eid].action_type.value for eid in references
                            if eid in event_by_id and eid in group.key_event_ids
                            and event_is_formal(event_by_id[eid])}
            for field in ("operation_title", "current_step", "observed_result", "physical_change"):
