...
def _candidate_timestamp(root: Path, reader: JsonLedgerReader) -> float:
    ...
    timestamps = [
        _parse_timestamp(status.get("updated_at"), float("-inf")),
        _parse_timestamp(metrics.get("run_ended_at"), float("-inf")),
        _parse_timestamp(submission.get("submitted_at"), float("-inf")),
    ]
    recorded = max(timestamps)
    return recorded if math.isfinite(recorded) else fallback
...
def discover_latest_candidate(
    ...
    for staging_name in ("Processing", ".VisionCortex-Run-Staging"):
        staging_parent = archive_root / staging_name / spec.archive_name
...
def analyze_three_datasets(
    ...
    return {
        ...
        "evidence_status": "PARTIAL_EVIDENCE" if passed else "NOT_PROVEN",
        "evidence_scope": "archived_receipts_only",
        "missing_release_gates": [
            "current deployed commit and environment verification",
            "real-media quality review and browser-visible delivery",
        ],
        "datasets": datasets,
        "summary": {
...
def render_acceptance_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "This read-only audit checks saved receipts; it does not establish current deployment or real-media quality.",
        "",
        "## Dataset Matrix",
        "",
