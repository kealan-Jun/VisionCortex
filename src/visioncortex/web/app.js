...
function setPhase(run) {
  ...
  const queue = state.deviceDayQueue || state.nasAutomaticProcessing?.queue || {};
  const automaticRunning = Object.values(queue).reduce((n, q) => n + (q.counts?.running || 0), 0);
  const automaticQueued = Object.values(queue).reduce((n, q) => n + (q.counts?.queued || 0), 0);
  const stage = run?.state || (automaticRunning ? "running" : "idle");
  chip.className = `phase-chip ${stage === "partial" ? "partial" : stage === "completed" ? "completed" : ["failed", "interrupted"].includes(stage) ? "failed" : run || automaticRunning ? "running" : ""}`;
  chip.querySelector("span").textContent = run?.parent_run_id ? (stage === "completed" ? "阶段结果已更新" : ["failed","interrupted"].includes(stage) ? "阶段更新未完成" : "正在处理所选阶段") : run ? (STAGE_LABELS[stage] || run.message || stage) : automaticRunning ? `自动采集处理：${automaticRunning} 个阶段任务运行中` : automaticQueued ? `自动采集处理：${automaticQueued} 个阶段任务等待处理` : "暂无进行中的任务";
}
...
async function loadNasRecordings() {
  ...
  try {
    ...
    state.nasAutomaticProcessing = payload.automatic_processing || null;
...
function deviceDaySummary(archive) {
  const rows = archive.recordings || [];
  const stages = rows.flatMap((row) => Object.values(row.stages || {}));
  const completed = rows.filter((row) => row.current_results_ready === true && ["retention", "vision", "stt", "understanding", "report"]
    .every((stage) => row.stages?.[stage]?.status === "completed")).length;
  const preprocessed = rows.filter((row) => row.preprocessing_ready === true).length;
  return {count: rows.length, completed, preprocessed, reportReady: completed > 0,
    running: stages.some((stage) => stage.status === "running"),
    failed: stages.some((stage) => stage.status === "failed")};
}

function deviceDaySection(reportsOnly = false, limit = Infinity, archives = state.deviceDayArchives || []) {
  const query = state.search.trim().toLocaleLowerCase("zh-CN");
  const rows = archives.filter((row) => (!reportsOnly || deviceDaySummary(row).reportReady) &&
    (!query || row.archive.toLocaleLowerCase("zh-CN").includes(query)));
  if (!rows.length) return state.deviceDaySyncError ? '<p role="status">设备日归档暂时无法同步，请稍后刷新。</p>' : "";
  const cards = rows.slice(0, limit).map((row) => {
    const summary = deviceDaySummary(row);
    const url = `#/device-day/${encodeURIComponent(row.archive)}`;
    const report = `${url}/report`;
    return `<article class="report-library-card"><header><span class="report-cover-mark">${icon(reportsOnly ? "file" : "folder")}</span><div><small>${esc(row.archive.slice(0,10))}</small><h2>${esc(row.archive.slice(11))}</h2></div><em>${summary.failed ? "有失败待处理" : summary.running ? "处理中" : summary.completed === summary.count ? "已归档分片处理完成" : "等待后续处理"}</em></header><p>已归档 ${number(summary.count)} 个采集分片，${number(summary.preprocessed)} 个预处理完成，${number(summary.completed)} 个全流程完成；保留 ${number(row.segment_count)} 个活动／无活动区间。</p><footer><a href="${url}/media">视频与 CSV</a><a href="${url}/clips">片段与关键帧</a><a href="${url}/comment">录音识别</a>${summary.reportReady ? `<a href="${report}">查看整合日报</a>` : ""}</footer></article>`;
  }).join("");
  return `<section class="device-day-library"><header class="panel-heading"><div><h2>${reportsOnly ? "设备日整合日报" : "NAS 设备日实验记录"}</h2><p>按采集日期和真实设备名查阅 · ${number(rows.length)} 份${state.deviceDaySyncError ? " · 同步中断，显示上次结果" : ""}</p></div><a href="#/device-days">全部设备日归档</a></header><div class="report-library-grid">${cards}</div></section>`;
}

function deviceDayQueueSection() {
  const labels = {retention:"原片与采集数据归档",vision:"YOLO 预处理",stt:"录音 STT",understanding:"多模态理解",report:"实验室日报"};
  const rows = Object.entries(state.deviceDayQueue || {});
  if (!rows.length) return "";
  return `<section class="panel"><header class="panel-heading"><h2>NAS 自动采集处理</h2><a href="#/capture">逐设备查看进度</a></header><div class="status-grid">${rows.map(([stage, value]) => statusCard("activity",labels[stage] || stage,`${number(value.counts?.running)} 个运行中`,`排队 ${number(value.counts?.queued)} · 完成 ${number(value.counts?.completed)} · 失败 ${number(value.counts?.failed)}`)).join("")}</div></section>`;
}
...
function renderHome() {
  ...
  const reportReady = state.archives.filter((archive)=>archive.has_daily_report).length + (state.deviceDayArchives || []).filter((archive)=>deviceDaySummary(archive).reportReady).length;
  const automaticRunning = (state.deviceDayArchives || []).filter((archive)=>deviceDaySummary(archive).running).length;
  ...
  main.innerHTML = `<div class="page workspace-page">
    ...
    <section class="home-focus-grid" aria-label="待办概览">
      ...
      <a href="#/tasks" class="${running.length + automaticRunning ? "active" : ""}"><span>${icon("activity")}</span><small>正在分析</small><strong>${number(running.length + automaticRunning)}</strong><em>${running.length + automaticRunning ? "查看任务进度" : "当前没有运行任务"} ${icon("arrow")}</em></a>
...
function nasBatchPicker() {
  ...
  const automatic = state.nasAutomaticProcessing?.enabled === true;
  const stageLabels = {retention: "原片留存", vision: "YOLO", stt: "录音 STT", understanding: "多模态理解", report: "日报"};
  const statusLabels = {waiting: "等待分片就绪", queued: "已自动入队", running: "正在处理", completed: "全流程已完成", failed: "有失败待处理", needs_camera_role: "待配置相机视角"};
  ...
  const rows = state.nasBatches.map((batch) => {
    const ready = batch.analysis_ready ?? batch.available;
    const status = automatic ? statusLabels[batch.processing?.status || "waiting"] : ready ? "可分析" : "等待文件就绪";
    const issue = batch.issues?.[0] || "";
    const progress = automatic ? (batch.processing?.recordings || []).map((record) => {
      const stages = Object.entries(stageLabels).map(([key, label]) => {
        const value = record.processing?.[key];
        const labelStatus = value?.status === "completed" ? "完成" : value?.status === "running" ? "运行中" : value?.status === "queued" ? "排队" : value?.status === "failed" ? "失败" : "待前序产出";
        return `<span>${label}：${labelStatus}${value?.wall_seconds != null ? ` · ${duration(value.wall_seconds)}` : ""}</span>`;
      }).join(" · ");
      const reportReady = record.processing?.report?.status === "completed";
      const base = record.archive ? `/api/device-days/${encodeURIComponent(record.archive)}` : "";
      const entries = record.processing?.retention?.status === "completed" && base
        ? `<p><a href="#/device-day/${encodeURIComponent(record.archive)}/clips">片段与关键帧索引</a> · <a href="#/device-day/${encodeURIComponent(record.archive)}/comment">录音识别</a>${reportReady ? ` · <a href="#/device-day/${encodeURIComponent(record.archive)}/report">查看当天日报</a>` : ""}</p>` : "";
      return `<li><strong>${esc(record.camera_key)}</strong> · ${formatDate(record.recording_start_time)}<p>${stages}</p>${entries}${record.issues?.length ? `<small>${esc(record.issues.join("；"))}</small>` : ""}</li>`;
    }).join("") : "";
    return `<article class="collection-card ${ready ? "ready" : "recording"}">
      <header><div title="采集编号：${esc(batch.recording_session_id)}"><small>${formatDate(batch.recording_start_time)}</small><strong>多视角采集 · ${number(batch.camera_count)} 个视角</strong></div><span class="collection-status ${ready ? "ready" : "recording"}">${status}</span></header>
      ...
      ${automatic ? `<p>已完成 ${number(batch.processing?.completed_slices || 0)} / ${number(batch.recording_count)} 个分片</p><details><summary>各设备、分片处理状态</summary><ul>${progress}</ul></details><a class="secondary-button" href="#/device-days">查看归档与处理队列</a>` : `<button class="primary-button" type="button" data-run-nas-batch="${esc(batch.batch_id)}" ${ready ? "" : "disabled"}>${icon("arrow")}分析此批次</button>`}
    </article>`;
  }).join("");
  const queue = automatic ? Object.entries(state.nasAutomaticProcessing.queue || {}).map(([stage, value]) => `<span>${stageLabels[stage] || esc(stage)}：排队 ${number(value.counts?.queued || 0)} · 运行 ${number(value.counts?.running || 0)} · 失败 ${number(value.counts?.failed || 0)}${value.backlog_warning ? ` · 积压，最早等待 ${duration(value.oldest_wait_seconds)}` : ""}</span>`).join("<br>") : "";
  const storage = state.nasAutomaticProcessing?.storage;
  const storageCopy = storage && !storage.ready ? `<p role="status">归档空间不足，新原片留存正在等待：可用 ${formatBytes(storage.available_bytes)}，本批次及处理预留需要 ${formatBytes(storage.required_bytes)}。采集端原片删除保持关闭。</p>` : "";
  return `<section class="form-section collection-picker" id="nas-batches"><div class="section-heading"><span>01</span><div><h2>${automatic ? "NAS 采集批次监控" : "选择采集批次"}</h2><p>${automatic ? "分片上传完成后自动处理。已关闭的短片和不完整分片也会进入流程，无需手动点击。" : "选择采集端已发布的视频分片，查看各阶段的处理状态和归档产物。"}</p><p class="nas-monitor-state ${monitorWatching ? "watching" : "retrying"}"><i></i>${esc(monitorCopy)}</p>${queue ? `<p class="collection-issues" role="status">${queue}</p>` : ""}${storageCopy}</div></div><div class="collection-card-list">${rows || `<div class="empty-state compact"><strong>${state.nasSyncError ? "暂时无法读取采集批次" : state.nasLoading ? "正在读取采集批次…" : "等待相机完成采集"}</strong></div>`}</div></section>`;
}
...
function bindNasBatchPicker() {
  document.querySelectorAll("[data-run-nas-batch]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (!(batch?.analysis_ready ?? batch?.available)) return;
...
function renderCaptureMonitor() {
  setChrome("capture", "NAS 采集监控");
  main.innerHTML = `<div class="page-heading"><div><p class="eyebrow">连续采集</p><h1>NAS 采集监控</h1><p>查看采集分片的自动处理进度，以及各设备当天的实验产出。</p></div><a class="secondary-button" href="#/device-days">设备日归档</a></div>${nasBatchPicker()}`;
  bindNasBatchPicker();
}
...
async function submitNasBatchRun(batch, button) {
  ...
  try {
    ...
    if (created.workflow === "device_day") {
      setProgress(.02, "分片已进入设备日归档队列，可在设备日归档查看各阶段产出");
      window.location.assign(created.archive_url);
      return;
    }
    state.activeRun = {
      run_id: created.run_id,
...
function renderOperations() {
  ...
  runtimePanel.innerHTML = "<h2>当前处理任务</h2><p>正在读取后台执行状态…</p>";
  main.querySelector(".page")?.append(runtimePanel);
  api("/api/runtime").then(runtime => {
    if (!runtimePanel.isConnected) return;
    const worker = runtime.worker || {};
    const labels = {running: "后台执行中", stopped: "后台已停止", not_started: "后台尚未启动", heartbeat_expired: "后台心跳超时", status_unavailable: "状态暂不可读"};
    const names = {vision: "视频预处理", storage: "资料归档", cpu: "分析与报告整理", cloud: "云端理解", stt: "录音识别"};
    const rows = runtime.resources || [];
    runtimePanel.innerHTML = `<h2>当前处理任务</h2><p>${esc(labels[worker.status] || "状态未知")} · ${esc(runtime.role === "web" ? "页面与后台独立运行" : "共用执行服务")}</p><p>${rows.length ? "" : "当前没有资源预约任务；可在任务进度中查看待处理任务。"}</p><ul>${rows.map(row => `<li>${esc(names[row.resource] || row.resource)} · ${esc(row.source === "nas" ? "自动采集" : "离线分析")} · ${esc(row.job)} · ${row.state === "running" ? "处理中" : "等待资源"}</li>`).join("")}</ul><p>版本 ${esc(runtime.build?.version || "未知")} · ${esc(runtime.build?.commit || "开发工作区")}</p>`;
  }).catch(() => { if (runtimePanel.isConnected) runtimePanel.textContent = "后台状态暂时无法读取，请刷新重试。"; });
...
function experimentCard(experiment, preliminary = false) {
  ...
  const title = experiment.workflow_kind === "continuous_workflow" ? experiment.name || "连续实验链" : objects.length ? `${objects.slice(0,3).join("、")}操作记录` : "实验操作记录";
...
function resultWorkspaceContent(data, tab, staging, notice, experimentResults) {
  ...
  return `<div class="page result-workspace result-workspace--${esc(tab)}"><p id="archive-sync-notice" class="analysis-readiness-note" role="status" hidden></p>${resultHeader(data,tab,staging)}${tab === "metrics" ? "" : status}${content}</div>`;
}
...
async function router() {
  ...
  if (route === "device-days" || route === "device-day") {
    ...
    if (route === "device-day" && parts[1]) return window.VisionCortexDeviceDays.renderDetail(ctx,parts[1],parts[2] || "report",parts[3] || "");
    return window.VisionCortexDeviceDays.renderLibrary(ctx);
  }
...
function refreshNasPickers() {
  if (!state.nasRenderPending || state.nasLoading || !["new", "capture"].includes(routeParts()[0])) return;
  let deferred = false;
...
async function refreshNasRecordings() {
  if (refreshingNasRecordings || state.nasLoading || document.hidden || !["new", "capture"].includes(routeParts()[0]) || state.health?.collection_ingest?.mode !== "directory_metadata") return;
