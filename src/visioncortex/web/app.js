...
function dailyReportView(data) {
  ...
  return `<section class="daily-report-cover"><div><p class="eyebrow">EVIDENCE-BACKED DAILY REPORT</p><h2>实验室日报 · ${esc(report.report_date)}</h2><p>${esc(report.experiment_id)}</p></div><span class="badge">${overview.evidence_package_eval_passed && qualityPassed ? esc(evidenceLevelLabel) : "待验收"}</span><div class="daily-report-links">${links.map(([key,label])=>`<a class="secondary-button" target="_blank" href="${esc(data.links[key])}">${icon("file")}${label}</a>`).join("")}</div></section>
