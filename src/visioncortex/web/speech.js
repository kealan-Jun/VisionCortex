    ...
    try {
      ...
      function showRows(result, append = false) {
        ...
        function rowButton(row, i) {
          ...
          button.innerHTML = `<time>${clock(row.start_seconds)}–${clock(row.end_seconds)}</time><span>${esc(row.text)}<small>${esc(row.view_id)} · 身份未确认 · ${esc(row.source_hint?.label || "声源未知")} · 录音提及${row.occurrence_count > 1 ? ` · 重复 ${row.occurrence_count} 次（首条提示）` : ""}</small></span><span aria-hidden="true">▶</span>`;
