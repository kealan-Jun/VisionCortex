window.VisionCortexDayTimeline = (() => {
  ...
  async function render({main,api,esc,setChrome,state}, day) {
    ...
    function showProgress(p) {
      ...
      const waitLabels={paused_by_user:'按用户要求暂停',prerequisite_not_verified:'等待前置产物校验或修复',upstream_failed:'上游失败待恢复',upstream_pending:'上游未完成',provider_blocked:'云端账户不可用',night_window:'等待夜间窗口',lease_recovery:'租约过期待恢复',pending_validation:'待校验调度'};
      ...
      ${selected?`<p>${selected.missing_input_count||0} 个输入缺失已从任务完成率分母中排除，保留失败记录；以后恢复时自动处理。</p><div class="processing-stage-grid">${Object.entries(stageNames).map(([key,label])=>{const c=selected.stages[key];return `<article class="processing-stage"><h4>${label}</h4><strong>${c.completed}<small> / ${selected.processing_total??selected.total} 分片任务完成</small></strong><progress value="${c.completed}" max="${Math.max(1,selected.processing_total??selected.total)}" aria-label="${label}任务完成比例"></progress><p><b>${c.running} 正在处理</b> · ${c.queued} 入队未完成 · ${c.waiting_for_prerequisite||0} 等待前置校验 · <span class="${c.failed?'processing-failure':''}">${c.failed} 失败</span> · ${(c.input_missing||0)+(c.input_waiting||0)+(c.input_unavailable||0)} 输入暂不可处理</p><small>${Object.entries(p.waiting?.[day]?.[key]||{}).map(([reason,n])=>`${n} ${waitLabels[reason]||esc(reason)}`).join(' · ')}</small><small>${c.not_enqueued?`${c.not_enqueued} ${pendingLabels[key]}`:'没有未入队分片'}${c.expired?` · ${c.expired} 待恢复租约`:''}</small></article>`;}).join('')}</div>`:'<p>该日期尚无采集队列记录，不能判断当天没有实验。</p>'}
