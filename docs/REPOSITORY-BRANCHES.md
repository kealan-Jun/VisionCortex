...
- `main`：保留现有稳定主线；产品集成提交不自动成为稳定发布。
- `codex/rtx4050-optimization-20260908`：客户源码更新和产品集成验收分支。

仓库：`https://github.com/RealityLoopAI/VisionCortex.git`。GitHub 默认显示 `main`，
查看客户更新时请切换到 4050 分支。拉取源码后仍需运行 `Update-From-Source.cmd`
更新原解压目录，详见 [RTX 4050 更新说明](RTX4050-GIT-UPDATES.md)。
...
这次统一客户更新地址是用户明确指定的集成分支操作。稳定 `main` 与正式发布仍需按
