...
模板源文件是 `src/labvision_evidence/templates/VC-LAB-DAILY-REPORT-V1.json`。每份日报保存模板 ID、版本与 SHA-256；评估器会核对固定栏目和模板哈希。需要调整栏目时应新增 V2，历史 V1 不回写、不覆盖。
...
```powershell
labvision generate-daily-report --archive .\outputs\clean-local-validation\exp_20260810_144014_e918b762-clean-validation --config .\configs\development-local.yaml
labvision serve --config .\configs\development-local.yaml --host 127.0.0.1 --port 8000
```
