...
- `src/visioncortex/schemas.py`: evidence and benchmark data models
- `src/visioncortex/liquid_state.py`: adapter registry, disabled backend,
  manifest builder, and deterministic evaluator
- `src/visioncortex/archive.py`: optional JSON embedding
- `src/visioncortex/indexing.py`: optional SQLite columns and filters
