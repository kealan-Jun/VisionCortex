...
原宿主实例仍存活时，相机监控线程已直接写入共享持久清单，companion 跳过
静态快照桥接，避免重复写回较旧的观察。原 NAS 宿主之后使用禁用配置重新启动，
仍会生成本地 `state/nas-recording-monitor.json`。此时 companion 只接收带时区、
不早于 `max(30秒, 3 × poll_seconds)` 且不超前五秒的成功快照，并保留原始
观察时间；重试及陈旧快照不被冒充新鲜的输入可用性证据。旧宿主仍存活时继续负责
