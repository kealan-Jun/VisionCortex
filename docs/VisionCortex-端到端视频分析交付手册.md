...
### 7.1 设备日视频理解的用量与防重复调用

2026-09-20 用户暂停视频多模态步骤理解和实验室日报；生产配置保持
`device_day.paused_stages: [understanding, report]`。原片归档、视频预处理与
Fun-ASR 继续独立处理，最新数据优先，空闲时补历史积压。该暂停不等于积压已清空。

设备日的两种理解入口共用逐窗复用逻辑：只接受通过原有完整校验的成功结果，
失败、缺帧、虚构帧引用或越界时间的窗口可在原队列有界重试内重新处理。并发
请求持有同一窗口锁；成功窗口不因后续窗口失败而重新付费。缓存绑定原图摘要、
时间、完整语义元数据、提示词和实际模型/图片/输出参数，历史用量与本次新增用量分开。
旧缓存只在完整 Input/Result 与哈希校验过的旧缓存共同吻合、模型/图片参数一致且
配置仍为旧默认输出预算 4096 时迁移；旧回执没有独立记录输出预算，不能用于证明
其他输出预算下的等价性。缺少这些证据的旧缓存不自动接受。历史失败输入和响应保留
在窗口的 `History/`，不会改写已封存回执。

查看本地视频请求账本（不调用模型、不读原视频，也不导入历史账单）：

```bash
python -m visioncortex.multimodal_usage \
  --config configs/rtx3090ti-ubuntu-production.yaml --date 2026-09-20
```

账本位于配置的本地 runtime 根目录 `state/MultimodalUsage.sqlite3`。日期采用上海
时间的请求预留日；跨午夜重试归原预留日。只统计接入新账本后的设备日视频调用，
不含手工实验入口、连接验证或语音识别。`known_tokens` 是服务端已报告用量，
`unknown_attempts` 和 `pending_calls` 不得当作零费用；缓存复用不创建付费条目。
此账本不是阿里云账单，人民币金额保持未知。

可在配置的 `mllm` 下设置以下选项，默认值为：

```yaml
compact_scene_metadata: false
scene_daily_attempt_limit: null
scene_daily_token_limit: null
```

调用次数上限在发送前原子预留 `max_retries` 个名额，收到回执后释放未用名额；
上限不足时保留排队任务，不消耗队列失败次数，不触发 STT 熔断。Token 上限是
**停止后续新请求的阈值**，单个请求可能超额，并非硬性 Token 或人民币封顶；
设置此阈值后，用量未知或仍在请求中的任务会阻止新视频请求，防止按零费用放行。
进程异常中断后没有回执的预留继续标记未知，当日不能凭猜测回收。未指定预算时
保持 `null`，不替用户设定金额。

输入压缩是待验收选项，默认关闭：全部图片、顺序、时间、覆盖说明、模型、思考
模式、输出预算和每帧观察仍保留；只从请求中移除文件路径、文件大小、哈希等存储
字段，并使用可还原的短帧编号。完整 Input、原始模型响应和映射仍在本地保存。
2026-09-20 对本地完成回执中 377 个窗口、10,678 帧的离线检查，帧元数据由
6,449,848 字符降至 2,207,348 字符（65.78%），语义字段与结构化引用还原通过。
这只证明表示转换，不证明总 Token 同比例减少或真实理解质量等价；两者均为
`NOT_PROVEN`，须在恢复视频调用后以相同真实视频做受控对照再启用。
没有降帧、缩图、关闭思考、替换模型或删减输出观察。供应商缓存折扣和批量价格
须按具体模型与账户核实，当前不假定支持。日报仍由本地模板生成，不另耗模型 Token。
