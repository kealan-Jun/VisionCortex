...
HTTP 服务退出先给已有请求 30 秒排空，避免慢读取无限阻止后台进入收尾。
Linux 工作线程退出时先允许任务正常收尾；60 秒后若仍有本进程直接拥有、仅向管道输出
原始帧的 FFmpeg 解码子进程，则通过 pidfd 定向终止，2 秒后仍未退出的强制回收。
不会按进程名批量结束其他服务，也不针对写文件的编码进程。被中断任务按既有租约
恢复机制续跑；已发布结果不重写。没有 Linux pidfd 的平台保留原退出机制。
包含后台工作线程的 systemd 服务使用 `KillMode=mixed` 和 180 秒退出上限：
先通知主进程收尾，超过上限再结束所属服务进程组。中断的未发布任务由持久队列
与既有发布日志恢复，不把被中断的执行记作成功。
