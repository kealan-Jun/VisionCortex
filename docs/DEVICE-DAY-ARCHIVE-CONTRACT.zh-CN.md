...
### 2026-09-17 内容查阅补充（用户明确授权）

最外层 `<日期>_<设备>` 和五大目录不变。`Comment` 与
`MultimodalUnderstanding` 增加直接按**采集绝对时间**组织的查阅入口：
...
已完成内容也进入日报及时间文件；整体状态明确为 `partial`。CV 的片段和索引
独立先发布，不等理解、日报或该投影刷新。

3090 Ti 从 **2026-09-17 20:00:00 +08:00 开始采集**的新片使用连续覆盖策略：
有活动区间顺序解码全部帧，按最多 48 帧/请求分组，全部提交图片理解；
无活动区间每 1 秒采样并包含区间首尾，按有界窗口覆盖全段，不设整段仅 8 帧的上限。
输入 JSON 的 `coverage` 保存时间范围、帧数、帧序号、最大间隔和采样策略。
顺序解码器时间不宣称为已验证原生 PTS；输入全帧不等于证明模型关注到了每一帧或准确理解了动作。
新策略只向视频理解提供画面，录音文字在独立 Transcript 文件中保留。
旧记录保持其实际抽帧回执，不自动重跑付费模型，不把旧结果重标为全帧理解。

从 **2026-09-17 20:45:00 +08:00** 起的新采集片段采用结构化视频步骤：
`experiment_steps` 保存动作、对象、前后状态、可见依据、源视频毫秒和绝对微秒。
静态场景、镜头移动、遮挡与重复姿态不单独充当实验步骤；无操作的输入返回空步骤。
旧版描述保留为 `scene_and_behavior_observations`，不能改名后宣称已经获得步骤级理解。
用户指定区间的单独复核保留输入、实际模型响应、采样覆盖和原回执引用；不触发整日历史补跑。
...
2026-09-24 用户授权取消原片归档作为新分片预处理的启动依赖。
已封口视频和对应已就绪音频分别直接进入 vision/STT，先封存输入内容和来源身份；
预处理、原片归档和正式索引发布具有独立持久状态。默认预处理优先，存在清理风险、
临近已知保留期限或等待老化的原片可以优先归档。未知外部保留期限必须报告缺口。
正式索引仍须等待对应 MetaVideo 内容校验与持久化成功；五目录及 v1 字段语义不变。
具体配置、增量迁移和恢复见 [NAS 原位预处理与独立归档](NAS-INPLACE-PREPROCESSING.zh-CN.md)。
下面的历史执行阶段名称继续保留，已执行旧任务维持其原有依赖和回执身份。
...
3. **stt**：与 YOLO 并行；新分片消费独立音频输入回执，历史任务继续消费原片留存回执。独立录音或原视频内嵌音轨必须进入已配置的 STT。3090 Ti 在 2026-09-17 恢复本地识别后，按用户随后恢复阿里云额度并要求提速的指令，选择 `fun-asr-flash-2026-06-15`；按实际录音时长分有界窗口，保存句段、词级时间、原音频引用、模型身份和执行回执，生成第五目录的机器comment。无音频保留 no_audio；模型识别结果为空保留 no_transcript，不能据此证明静音；只有明确的语音活动检测结果才可标 no_speech，不伪造模型调用或识别文本。STT失败不占用或阻止YOLO工位。
...
五个执行阶段使用独立 SQLite WAL 持久化队列，保存在配置的本地 runtime 根目录；NAS 仍为五个产出目录。历史任务依赖关系为 retention → vision、retention → stt；新分片为 input-vision → vision、input-stt → stt，归档独立调度，验证归档与预处理结果后正式发布，再进入 understanding → report。文件级互斥、任务租约和原子文件发布支持重启续跑。完成且输入、代码、模型及产物校验一致的阶段复用；YOLO 扫描和多模态响应另以实际输入及模型身份校验复用，缓存复用与本轮新模型调用分别记录；录音晚到不会重新运行不变视频的YOLO，comment/protocol/STT变化使理解与日报重新产出。失败保留为 failed，付费模型请求不会在每轮轮询无穷重试。
...
当时生产配置固定 `speech_recognition.provider=aliyun_qwen`、`model=qwen-audio-3.0-asr-flash`；不静默回退到 Whisper 或旧 Qwen 模型。此生产选择已由下述 2026-09-17 明确变更取代，Qwen 适配器与历史回执继续保留。Qwen 录音按实际时长划分最长 180 秒的识别窗口，原录音不切改；16 kHz 单声道派生音频通过 Base64 请求发送给已绑定的 Aliyun 服务，无需公开 NAS 文件。保存句段与词级原录音时间、派生字幕时间、模型名称、适配器 SHA-256、原始响应、请求 ID、用量及执行耗时。流式累计文本按供应商最终时间戳去重，未完成的句子或缺失时间戳不能发布。空识别结果保留来源，不能冒充没有人说话。模型别名的服务端权重无法在本地验证；真实转写准确性仍需人工复核。
