...
增强策略本身可使用 `annotation-workbench-augmentation/1` 或 `/2`；它与上述部分监督导出版本是两个独立字段。
策略 v2 必须显式给出 `horizontal_flip_variants`（0起始、不重复、小于每图版本数），消费方核对每个版本的翻转选择、反射矩阵、全部已知框与 ignore。
保留基础参数时外观种子与 v1 一致，未翻转版本像素可复现；完整派生 ID 仍改变，不能忽略由此导致的样本排序差异。
默认策略和在线增强保持原设置；这项能力不自动启用，也不证明左右翻转适合每个项目或已经改善真实视频。
