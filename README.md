...
阶段成果独立保存：每次阶段完成后先保存产物，再发布完成记录与根目录的
`阶段产出清单.json`。`JSON-Config-Files/Stage-Versions/` 保留当时的元数据副本和 SHA-256；
视频仅保留路径及文件属性引用，不额外复制或重复全片校验。后续精炼不会改写这些元数据版本。
网页可选各已完成环节查看文件、完成记录与版本索引；旧任务没有保存的历史快照不会补造。

阶段恢复与失败隔离：任务记录提供“继续未完成环节”。新任务逐阶段保存身份绑定的恢复点；
恢复时重新检查环境、输入、代码、配置、模型与产物，跳过校验通过的已完成环节。
旧任务没有恢复点时仍需执行一次，不会仅凭“已完成”字样跳过计算。
录音、采集质量检查、片段导出与关键素材之间按实际依赖隔离；必要依赖失败时标记“等待依赖”，
保留其余结果。日报与专业 PDF 分开生成；完整发布仍须通过质量检查及归档门禁。

页面默认跟随新完成环节，手动切换页面或播放视频时让用户继续查看，不中止后台任务。
本地计算并同步 NAS 的配置会保留归档重试清单；暂时断连不会取消已完成计算，
但未完成同步不能显示正式归档成功。NAS 直接计算模式仍依赖该文件系统可写。
[阶段恢复与归档契约](docs/STAGE-RECOVERY.md)。

活动分类与动作证据分开：单独器材整理、清洁或准备活动保留视频、步骤和溯源，但不计入实验数；
与后续实验有承接的准备步骤仍在实验链中。分类无法确认时保留待确认状态。
用户复核存入 `JSON-Config-Files/Activity-Reviews/`，绑定原输入与片段身份；
不改变原模型记录、动作证据或质量验收决定。新模型分类的实际准确率仍需真实视频验收。

Windows SAM2 缓存使用本地缓存根目录下的完整指纹，不再叠加实验、事件和机位长名称。
...
录音为可选环节：未启用或无可用音轨时，环节回执显示“已跳过”，视频分析继续；
默认 `speech_recognition.required: false` 时，转写或录音时间线失败只记录该环节失败，
不会阻塞独立的视频处理。失败的部分转写不进入模型证据上下文；需强制转写成功的
配置可设 `required: true`。原始输入、视频证据和正式报告质量门仍独立校验。
...
新任务默认跟随已保存的阶段产出，完成后打开对应结果页；手动切换页面会暂停跟随，
顶部按钮可恢复。查看与播放是只读请求，不取消持久队列中的分析任务。实验片段完成时
立即发布视频文件夹与分组映射，后续关键素材理解期间可播放；不会展示仍在写入的片段。
阶段列表依据实际回执区分处理中、完成、跳过和失败，缺少回执不会冒充已归档。
本次输出根目录的 `阶段产出清单.json` 随各环节实际文件保存/发布后更新，包含文件相对路径、
状态、原因和时间；它不是正式发布凭证，也不混入旧尝试的阶段回执。
NAS 批次展示所有阻塞原因以及各相机采集端报告的覆盖率和帧间隔；已关闭但质量未通过的
采集显示“需要处理”，不会一直显示“采集中”。这些采集指标不代表实验识别准确率；
相机视角仍需明确配置，数据完整性门不因缺少可用批次而放宽。
