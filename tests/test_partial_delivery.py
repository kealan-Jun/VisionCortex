...
@pytest.mark.parametrize("terminal_state", ["failed", "partial"])
def test_registered_local_result_survives_restart_without_publishing_or_enqueueing(tmp_path, monkeypatch, terminal_state):
    ...
    write_json(root / "run_status.json", {"stage": terminal_state, "failed_stage": "package"})
    ...
    write_partial_delivery(root, {}, analysis_finished=terminal_state == "partial")
    ...
    if terminal_state == "partial":
        ...
        report.write_bytes(report_bytes + b"changed")
...
def test_refresh_queue_is_durable_and_excludes_parent_retry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(api, '_queue_stop', Event())
