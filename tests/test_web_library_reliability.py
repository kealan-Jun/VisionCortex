...
def run_javascript(names, script):
    ...
    functions = []
    dependencies = {
        "loadAll": ("loadDeviceDayArchives",),
        "refreshTaskSnapshots": ("loadDeviceDayArchives",),
...
def test_nas_monitor_shows_automatic_stage_progress_and_results():
    run_javascript(("nasBatchPicker",), r'''
const state={nasSyncError:false,nasMonitor:{status:"watching"},nasBatches:[{batch_id:"B",camera_count:1,recording_count:1,processing:{status:"completed",completed_slices:1,recordings:[{recording_id:"R",camera_key:"a_cam01",archive:"2026-09-10_a_cam01",archive_url:"/report",processing:{retention:{status:"completed",wall_seconds:2},vision:{status:"completed",wall_seconds:10},report:{status:"completed"}}}]}}],nasAutomaticProcessing:{enabled:true,queue:{vision:{counts:{queued:3,running:2},backlog_warning:true,oldest_wait_seconds:1000}}}};
const esc=String,icon=()=>"",number=Number,formatDate=String,duration=String,formatBytes=String;
const html=nasBatchPicker();
assert.ok(html.includes("NAS 采集批次监控") && html.includes("无需手动点击"));
assert.ok(!html.includes("data-run-nas-batch"));
assert.ok(html.includes("YOLO：完成") && html.includes("积压"));
assert.ok(html.includes("查看当天日报") && html.includes("录音识别") && html.includes("片段与关键帧索引"));
''')
