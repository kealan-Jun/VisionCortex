...
def test_semantically_proven_container_transition_builds_complete_state_receipt(
    default_config,
):
    event = _event(
        "CAP-STATE",
        _candidate(
            ...
            ["gloved_hand", "bottle_cap", "sample_bottle"],
            [{"distance_norm": 0.0}],
        ),
    )
    ...
    assert receipt["action_type"] == "container_state_change"
    assert receipt["action_subtype"] == "cap_remove_or_replace"
    assert receipt["phases"] == receipt["required_phases"]
    assert receipt["lifecycle_state"] == "completed"
    assert receipt["state_after"] == {"container": "observed_changed_state"}
