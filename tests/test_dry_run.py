...
def test_dry_run_builds_contract_without_video_or_ffmpeg(tmp_path):
    ...
    category_index = json.loads(
        (output / "Key-Materials" / "Key-Material-Category-Index.json").read_text(
            encoding="utf-8"
        )
    )
    assert category_index["category_count"] == 5
    assert category_index["experiments"][0]["action_categories"][0]["event_count"] == 1
    assert list(
        (
            output
            / "Key-Materials"
            / "Key-Frames"
            / "001_Pipetting-Operation-Experiment"
    ...
    assert (
        index_manifest["files"]["key_material_category_index"]
