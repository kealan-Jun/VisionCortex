...
def test_scene_timing_contract_reaches_provider_and_receipt(default_config, monkeypatch, scene):
    from copy import deepcopy
    from visioncortex.mllm import SCENE_TIME_REFERENCE_RULES

    config = deepcopy(default_config)
    config['mllm'].update(selection(), enabled=True, max_retries=1)
    monkeypatch.setattr('visioncortex.mllm.model_api_key', lambda *args, **kwargs: 'synthetic-only')
    metadata = {'schema_version': 'visioncortex-device-day/1', 'start_ms': 100533,
                'end_ms': 102000, 'frames': [{'frame_id': 'a', 'local_ms': 100533.333},
                                          {'frame_id': 'b', 'local_ms': 101066.666}]} if scene else {}
    original = deepcopy(metadata)

    def handler(request):
        body = json.loads(request.content)
        system = body['messages'][0]['content']
        assert (SCENE_TIME_REFERENCE_RULES in system) is scene
        return streamed_response(json.dumps({'summary': 'synthetic response'}))

    analyzer = factory_for(handler)(config)
    try:
        result = analyzer._call('original evidence rules', metadata, [])
    finally:
        analyzer.close()
    assert result['status'] == 'completed'
    assert ('scene_timing_contract' in result) is scene
    if scene:
        assert result['scene_timing_contract']['instructions'] == SCENE_TIME_REFERENCE_RULES
        assert result['scene_timing_contract']['validation_relaxed'] is False
