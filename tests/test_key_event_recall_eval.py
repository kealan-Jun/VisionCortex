...
def test_key_event_recall_is_one_to_one_object_aware_and_coverage_scoped():
    ...
    assert report["cv_prediction_count_in_scope"] == 2
    ...
    assert report["cv_only_threshold_results"][1]["recall"] == 0.5
...
def test_recall_excludes_semantically_reviewed_unconfirmed_prediction():
    event = prediction(
        "P-UNCERTAIN-LIQUID",
        ActionType.LIQUID_MOVEMENT,
        10_000,
    ...
    report = evaluate_key_event_recall(
        [event],
        {
            "ground_truth_id": "semantic-filter-test",
            "events": [
                {
                    "event_id": "GT-CONTACT",
                    "action_type": "hand_object_contact",
    ...
    assert report["prediction_count_in_scope"] == 0


def test_recall_retains_direct_action_class_when_one_view_is_explicitly_supported():
    event = prediction(
        "P-DIRECT-CONTACT",
        ActionType.HAND_OBJECT_CONTACT,
        10_000,
    ...
    event.model_understanding = {
        ...
        "per_view_observations": [
            {
                "view_id": "fp",
                "observation": "The gloved hand visibly grasps the weighing paper.",
            }
        ],
    }
    report = evaluate_key_event_recall(
        [event],
        {
            "ground_truth_id": "direct-class-test",
            "events": [
                {
                    "event_id": "GT-CONTACT",
                    "action_type": "hand_object_contact",
    ...
    assert report["direct_action_class_supported_prediction_ids"] == [
        "P-DIRECT-CONTACT"
    ]
    assert report["threshold_results"][1]["recall"] == 1.0


def test_recall_keeps_indirect_state_classes_strict_even_with_same_model_class():
    event = prediction(
        "P-UNCERTAIN-PANEL",
    ...
    event.model_understanding = {
        "status": "completed",
        "per_view_observations": [
            {"view_id": "fp", "observation": "The balance is visible."}
        ],
    }
    report = evaluate_key_event_recall(
        [event],
        {
            "ground_truth_id": "strict-indirect-test",
            "events": [
                {
                    "event_id": "GT-CONTACT",
                    "action_type": "hand_object_contact",
    ...
    assert report["excluded_semantically_unconfirmed_prediction_ids"] == [
        "P-UNCERTAIN-PANEL"
    ]
