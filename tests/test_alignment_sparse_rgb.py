...
def test_bounded_reader_recovers_rgb_rows_between_sampling_strides(tmp_path):
    ...
    # The unified reader now strides only eligible RGB rows.
    assert len(read_timestamp_csv(path, 10, max_points=5)) >= 2
    points = read_timestamp_csv_bounded(path, 10, sample_count=5)
    assert points == read_timestamp_csv(path, 10, max_points=5)
