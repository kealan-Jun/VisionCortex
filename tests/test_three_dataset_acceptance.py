...
from visioncortex.cli import app
from visioncortex.three_dataset_acceptance import (
    DatasetSpec,
...
def test_read_only_acceptance_passes_without_mutating_archive(tmp_path: Path):
    ...
    assert "Production release ready: **NO**" in render_acceptance_markdown(result)


@pytest.mark.parametrize("staging_name", ["Processing", ".VisionCortex-Run-Staging"])
def test_latest_failed_staging_is_not_hidden_by_old_formal_archive(tmp_path: Path, staging_name):
    archive_root = tmp_path / "archive"
    formal = archive_root / "Six"
    staging = archive_root / staging_name / "Six" / "run-new"
...
def test_cli_writes_json_and_markdown_outside_archive(tmp_path: Path):
    ...
    assert "\"production_release_ready\": false" in response.output.lower()
