...
def augmented_partial_fixture(tmp_path, reflection=None):
    import copy
    ...
    if reflection is not None:
        policy.update(schema_version="annotation-workbench-augmentation/2",
                      horizontal_flip_variants=[0] if reflection else [])
    ...
    tx = 7 if reflection is not None else 10  # Asymmetric padding exposes stale ignore coordinates.
    mx, offset = (-.8, 100 - tx) if reflection else (.8, tx)
    for box in boxes + regions:
        x1, y1, x2, y2 = box["xyxy_px"]
        if reflection:
            x1, x2 = x2, x1
        box["xyxy_px"] = [x1 * mx + offset, y1 * .8 + 8, x2 * mx + offset, y2 * .8 + 8]
    d = dict(id=iid, source_image_id=row["id"], source_revision=1, source_path=s["path"],
             ...
             source_to_output_matrix=[[mx, 0, offset], [0, .8, 8], [0, 0, 1]], output_size=[100, 80],
    ...
    if reflection is not None:
        d.update(values=dict(horizontal_flip=reflection),
                 appearance_identity_sha256=canonical_sha(dict(identity, policy=base_policy)))
        d["operation_order"].insert(-1, "horizontal_flip_if_selected")
    im = Image.new("RGB", (100, 80), (114, 114, 114))
    with Image.open(s["path"]) as original:
        im.paste(original.resize((80, 64)), (tx, 8))
    if reflection:
    ...
    metadata[d["image_file"]] = dict(image_id=iid, source_size=[100, 80],
                                     annotation_sha256=d["derived_annotation_sha256"], xyxy_px=[r["xyxy_px"] for r in regions])
    (root / "ignore-regions.json").write_text(json.dumps(metadata))
    (root / "annotations.json").write_text(json.dumps(dict(images=rows)))
...
def test_explicit_reflection_export_preserves_sampling_and_unknown_regions(tmp_path, reflection):
    root, receipt = augmented_partial_fixture(tmp_path, reflection=reflection)
    _, rows = validate_export(root, sha256(root / "receipt.json"), "first_person", supervision="outside_ignore")
    sources = training_sampling_sources(root, "first_person", receipt, rows)
    assert len(sources) == 2 and {r["source_group"] for r in sources.values()} == {"recording-0"}
    d = receipt["augmentation"]["records"][0]
    assert d["ignore_regions"][0]["xyxy_px"] == ([37, 32, 69, 56] if reflection else [31, 32, 63, 56])


@pytest.mark.parametrize("mutation", [
    lambda d: d["values"].update(horizontal_flip=False),
    lambda d: d.update(values=None),
    lambda d: d["values"].update(horizontal_flip=1),
    lambda d: d.update(appearance_identity_sha256="0" * 64),
    lambda d: d.update(source_to_output_matrix=[[.8, 0, 7], [0, .8, 8], [0, 0, 1]]),
    lambda d: d["ignore_regions"][0].update(xyxy_px=[31, 32, 63, 56]),
    lambda d: d["boxes"][0].update(xyxy_px=[7.8, 9.6, 23, 32]),
])
def test_rehashed_reflection_cannot_hide_stale_geometry_or_false_policy(tmp_path, mutation):
    from visioncortex.project_augmentation import yolo_labels

    root, receipt = augmented_partial_fixture(tmp_path, reflection=True)
    d = receipt["augmentation"]["records"][0]
    mutation(d)
    d["derived_annotation_sha256"] = canonical_sha({k: d[k] for k in ["boxes", "ignore_regions"]})
    metadata = json.loads((root / "ignore-regions.json").read_text())
    metadata[d["image_file"]].update(annotation_sha256=d["derived_annotation_sha256"],
                                      xyxy_px=[r["xyxy_px"] for r in d["ignore_regions"]])
    (root / "ignore-regions.json").write_text(json.dumps(metadata))
    (root / d["label_file"]).write_text(yolo_labels(d["boxes"], 100, 80))
    d["label_sha256"] = sha256(root / d["label_file"])
    for name in receipt["files"]:
        receipt["files"][name] = sha256(root / name)
    (root / "receipt.json").write_text(json.dumps(receipt))
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize("version, indices", [
    ("1", [0]), ("2", [-1]), ("2", [1]), ("2", [0, 0]), ("2", [True]), ("2", "0"),
])
def test_invalid_reflection_policy_rejected_before_derivative_use(tmp_path, version, indices):
    root, receipt = augmented_partial_fixture(tmp_path, reflection=True)
    receipt["augmentation"]["policy"].update(
        schema_version=f"annotation-workbench-augmentation/{version}", horizontal_flip_variants=indices)
    (root / "receipt.json").write_text(json.dumps(receipt))
    with pytest.raises(ValueError, match="reflection|Reflection"):
        validate_export(root, sha256(root / "receipt.json"), "first_person", supervision="outside_ignore")
